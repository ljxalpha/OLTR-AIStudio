...
def run(train_set, test_set, ranker, num_interation, click_model):
    ...
    for i in index:
        qid = query_set[i]
        # 这里就是返回打分后的结果列表（长度不超过10），以及与当前query关联的所有文档的分数列表
        result_list, scores = ranker.get_query_result_list(train_set, qid)
        # 三个返回值：已点击文档列表，所有文档的点击标签，用户是否被满足
...
def job(model_type, f, train_set, test_set, tau):
    ...
    # for r in range(1, 3):
        ...
        if (not os.path.exists("../results/exploration/mq2007/PDGD/fold{}".format(f))):
            os.makedirs("../results/exploration/mq2007/PDGD/fold{}".format(f))
